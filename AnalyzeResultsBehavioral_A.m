...
for SubjInd = 1:length(Subjects)
    ...
    copyfile('Logfile_*Run_100*.txt', 'ALoc')
    copyfile('Trial_List_Subject_*Run_100*.txt', 'ALoc')
    copyfile('Side_List_Subject_*Run_100*.txt', 'ALoc')

    cd(fullfile(StartDirectory, 'Subjects_Data', strcat('Subject_', num2str(Subjects(SubjInd))), 'ALoc'))

    LogFileList = dir(strcat('Logfile_Subject_', num2str(Subjects(SubjInd)), '_Run_100*.txt'));
    ...
    for iFile=1:size(Data,2)
        ...
        ylabel(['% correct on run ' num2str(iFile)]);
